"""
Data loading and analysis utilities for Graph Transformers project.
"""
...
# Handle PyTorch 2.6+ weights_only security change for OGB datasets
torch.serialization.add_safe_globals(
    [
        torch_geometric.data.data.DataEdgeAttr,  # Edge attributes (bond types, weights)
        torch_geometric.data.data.DataTensorAttr,  # Tensor attributes
        torch_geometric.data.data.Data,  # Main graph data structure
        torch_geometric.data.storage.GlobalStorage,  # Global data storage
        torch_geometric.data.storage.NodeStorage,  # Node data storage
        torch_geometric.data.storage.EdgeStorage,  # Edge data storage
    ]
)
...
def load_peptides_func_dataset(root="./data"):
    """Load Peptides-func dataset from LRGB (Long Range Graph Benchmark)"""
...
def analyze_graph_properties(graphs):
    """Analyze properties of a list of graphs"""
    num_nodes = []
    num_edges = []
    node_degrees = []
...
def print_dataset_summary(dataset, sample_size=50):
    ...
    print("=" * 60)
    # Handle datasets that don't have a name attribute
    ...
    print(
        f"Avg nodes: {np.mean(props['num_nodes']):.1f} ± {np.std(props['num_nodes']):.1f}"
    )
    print(
        f"Avg edges: {np.mean(props['num_edges']):.1f} ± {np.std(props['num_edges']):.1f}"
    )
    print(
        f"Avg degree: {np.mean(props['node_degrees']):.1f} ± {np.std(props['node_degrees']):.1f}"
    )
